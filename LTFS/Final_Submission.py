...
outfile=r'C:\PraveenT\PycharmProjects\Trial\Analytics Vidhya\Hackathon\LTFS\FinalSubmission.csv'
...
df['weekend']=pd.DatetimeIndex(df['application_date']).dayofweek
df['monsoon']=pd.DatetimeIndex(df['application_date']).month
...
dict={'New Years Day':1,'Makar Sankranti  Pongal':2, 'Republic Day':3,
 ...
 'Eid-ul-Fitar':13, 'Janmashtami':15, 'Independence Day':16,
...
fun=lambda x:1 if(x==9)|(x==10)|(x==11) else(0)
fun1=lambda x:1 if(x==5)|(x==6) else(0)
fun2=lambda x:1 if(x==6)|(x==7)|(x==8) else(0)
df_seg_01['festmonth']=df_seg_01['festmonth'].apply(fun)
df_seg_02['festmonth']=df_seg_02['festmonth'].apply(fun)
df_seg_01['weekend']=df_seg_01['weekend'].apply(fun1)
df_seg_02['weekend']=df_seg_02['weekend'].apply(fun1)
df_seg_01['monsoon']=df_seg_01['monsoon'].apply(fun2)
df_seg_02['monsoon']=df_seg_02['monsoon'].apply(fun2)

grouped_df_01 = df_seg_01.groupby(['year','month','day','dayofweek','quarter','dayofyear','weekofyear','monthstart','monthend','quarterstart','quarterend','yearstart','yearend','festmonth','weekend','monsoon','HOLIDAY']).agg({'case_count': sum})
grouped_df_02 = df_seg_02.groupby(['year','month','day','dayofweek','quarter','dayofyear','weekofyear','monthstart','monthend','quarterstart','quarterend','yearstart','yearend','festmonth','weekend','monsoon','HOLIDAY']).agg({'case_count': sum})
...
index = int(round(grouped_df_01.shape[0]*0.99-1,1))
...
index = int(round(grouped_df_02.shape[0]*0.99-1,1))
...
sub['weekend']=pd.DatetimeIndex(sub['application_date']).dayofweek
sub['monsoon']=pd.DatetimeIndex(sub['application_date']).month
...
sub_01['weekend']=sub_01['weekend'].apply(fun1)
sub_02['weekend']=sub_02['weekend'].apply(fun1)
sub_01['monsoon']=sub_01['monsoon'].apply(fun2)
sub_02['monsoon']=sub_02['monsoon'].apply(fun2)
